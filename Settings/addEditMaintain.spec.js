...
describe('Fleet Login Tests', function () {
    ...
    it('should open List Of Devices modal, select Standard option, and verify warning', async function () {
        try {
            ...
            const trackInfoLink = await waitForClickable(driver, By.id('aAddEditMaintContact'));
            await clickWithRetry(driver, trackInfoLink);
            
            // Wait for modal to appear
            const modal = await waitForClickable(driver, By.id('divMaintContact'));
            expect(await modal.isDisplayed()).to.be.true;

            // Find the email input field and enter test email
            const emailInput = await waitForClickable(driver, By.css("input#txtEmail"));
            await emailInput.clear();  // Clear any existing text
            await emailInput.sendKeys('testaut@gmail.com');
            
            // Find and click the Add Email button
            const addEmailButton = await waitForClickable(driver, By.id("btnAddMaintEmail"));
            await clickWithRetry(driver, addEmailButton);

            // Wait a moment for the email to be added
            await driver.sleep(2000);
            
            // Wait for the email to appear in the list (adjust the selector based on your actual implementation)
            await driver.wait(
                until.elementLocated(By.xpath("//label[contains(text(), 'testaut@gmail.com')]")),
                10000
            );
            
            // Verify the email was added
            const addedEmail = await driver.findElement(By.xpath("//label[contains(text(), 'testaut@gmail.com')]"));
