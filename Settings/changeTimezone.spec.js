...
describe('Fleet Login Tests', function () {
    ...
    it('should open List Of Devices modal, select Standard option, and verify warning', async function () {
        try {
            ...
            const trackInfoLink = await waitForClickable(driver, By.id('aChangeTimezone'));
            await clickWithRetry(driver, trackInfoLink);
            
            // Wait for modal to appear
            const modal = await waitForClickable(driver, By.id('divChangeTimezone'));
            expect(await modal.isDisplayed()).to.be.true;

            // Find and click the Standard radio button
            const standardRadio = await waitForClickable(driver, By.css("input[type='radio'][id='optradio-8']"));
